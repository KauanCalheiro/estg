...
Nota 2: é utilizado um identificador para distinguir o código que é específico a
cada thread.
*/
...
typedef struct {
    ...
    pthread_mutex_t
        *mutex; /* ponteiro para um mutex a partilhar por todas as threads */
    pthread_cond_t *cond; /* ponteiro para uma variável de condição a partilhar
...
int main(int argc, char *argv[]) {
    ...
    pthread_mutex_t
        mutex; /* mutex que vai ser 'partilhado' por todas as threads */
    pthread_cond_t cond; /* variável de condição que vai ser 'partilhada' por
                            todas as threads */
    ...
    for (int i = 0; i < MAX_THREADS; i++) {
        if ((errno = pthread_create(&tids[i], NULL, task, &thread_params[i])) !=
            0)
...
void *task(void *arg) {
    ...
    if (params->id == 1) {
        printf(
            "Thread %d - sinalizará a variável de condição daqui a 5 "
            "segundos\n",
            params->id);
        sleep(5);
    }
    ...
    /* a primeira thread sinaliza uma variável de condição, as restantes esperam
     * nessa variável */
    if (params->id == 1) {
