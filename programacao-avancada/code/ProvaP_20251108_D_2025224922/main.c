...
void app(struct gengetopt_args_info args)
{
    ...
    {
        int chunk_size = base + (i < rem ? 1 : 0);

        patial_thread_data[i].chunk_vector = (chunk_size > 0) ? &vector[offset] : NULL;
        patial_thread_data[i].thread_id = i;
        patial_thread_data[i].size = chunk_size;
        patial_thread_data[i].partial_sum = NULL;

        if ((errno = pthread_create(&patial_thread[i], NULL, thread_partial_sum, &patial_thread_data[i])) != 0)
        {
            ERROR(EXIT_FAILURE, "pthread_create() failed!");
        }
    ...
    {
        if ((errno = pthread_join(patial_thread[i], NULL)) != 0)
        {
            ERROR(EXIT_FAILURE, "pthread_join() failed!");
        }
    }
    ...
    for (int i = 0; i < args.threads_arg; i++)
    {
        final_sum += *patial_thread_data[i].partial_sum;
        free(patial_thread_data[i].partial_sum);
    }

    soma_final(vector, args.size_arg, final_sum);
    return;
}
...
void *thread_partial_sum(void *arg)
{
    ...
    printf("thread[%d]: inicia processamento\n", data->thread_id);
    ...
    *sum = 0;
    ...
    for (int i = 0; i < data->size; i++)
    {
        *sum += data->chunk_vector[i];
        printf(
            "thread[%d]: soma array[%d]=%d -> acumulado=%d\n",
            data->thread_id,
            i,
            data->chunk_vector[i],
            *sum
...
{
    int sum = 0;
    printf("main: inicia verificação sequencial\n");
