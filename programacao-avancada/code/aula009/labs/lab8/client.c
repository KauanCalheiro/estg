...
int main(int argc, char *argv[]) {
    ...
    ser_addr.sin_port = htons(args_info.port_arg);

    /* Utiliza a função inet_pton para preencher o endereço */
    switch (inet_pton(AF_INET, args_info.ip_arg, &ser_addr.sin_addr)) {
    case 0:
        printf("O endereço IP %s não e' valido\n\n", args_info.ip_arg);
        cmdline_parser_print_help();
        return 2;
    case -1:
        printf("Endereço IP desconhecido: %s\n\n", args_info.ip_arg);
        cmdline_parser_print_help();
        return 2;
    }
